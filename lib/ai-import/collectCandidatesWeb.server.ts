...
async function loadTodaySearchedSubjectIds(
  ...
  if (error) {
    throw new Error(`Failed to load recent import runs: ${error.message}`);
  }

  const searched = new Set<string>();
  for (const row of (data ?? []) as any[]) {
    const startedAt = typeof row.started_at === "string" ? row.started_at : "";
    if (!startedAt || getKstDateKey(new Date(startedAt)) !== dateKey) continue;

    const selectedSubjects = row.stats?.selectedSubjects;
    if (!Array.isArray(selectedSubjects)) continue;

    for (const subject of selectedSubjects) {
      const id = typeof subject?.id === "string" ? subject.id : "";
      if (id) searched.add(id);
    }
  }
...
function priorityForSubject(subject: Subject, dateKey: string) {
  ...
  const activity =
    age <= 7 ? 40 : age <= 21 ? 30 : age <= 60 ? 18 : age <= 120 ? 8 : 0;
  const coverage =
    subject.workCount === 0
      ? 24
...
function selectSubjects(
  ...
  for (const category of CATEGORIES) {
    const categorySubjects = subjects
      .filter(
        (subject) =>
          subject.category === category && !excludedSubjectIds.has(subject.id),
      )
      .map(
        (subject): Subject => ({
          ...subject,
          priorityScore: priorityForSubject(subject, dateKey),
        }),
      )
...
function buildSearchQuery(subject: Subject) {
  const primary = subject.nameKo || subject.nameEn || subject.nameZhTw;
  const localized =
    subject.category === "cheer"
      ? `${primary} 치어리더 직캠`
      : `${primary} 직캠`;
  return `${localized} ${EXCLUDED_SEARCH_TERMS.join(" ")}`.trim();
}

function collectYoutubeIdsFromHtml(html: string) {
  const ids: string[] = [];
  ...
  for (const pattern of patterns) {
    for (const match of html.matchAll(pattern)) {
      add(match[1]);
      if (ids.length >= SEARCH_RESULTS_PER_SUBJECT) return ids;
    }
  }
...
async function searchYouTubeWeb(subject: Subject): Promise<string[]> {
  const params = new URLSearchParams({
    search_query: buildSearchQuery(subject),
    hl: "ko",
    gl: "KR",
    sp: "CAI%3D",
  });
    ...
    const response = await fetch(
      `https://www.youtube.com/results?${params.toString()}`,
      {
        ...
        headers: {
          ...
          cookie: "PREF=hl=ko&gl=KR; SOCS=CAI",
        },
      },
    );

    if (!response.ok) {
      throw new Error(`YouTube web search returned ${response.status}.`);
    }

    const html = await response.text();
    if (
      html.length < 20_000 ||
      /unusual traffic|consent\.youtube\.com|before you continue to youtube/i.test(
        html,
      )
    ) {
      throw new Error("YouTube web search returned a blocked/consent page.");
    }

    const ids = collectYoutubeIdsFromHtml(html);
    if (ids.length === 0) {
      throw new Error("YouTube web search parser returned no video IDs.");
    }
    return ids;
  } finally {
...
async function searchYouTubeApiFallback(subject: Subject): Promise<string[]> {
  if (!YOUTUBE_API_KEY) {
    throw new Error("YOUTUBE_API_KEY is not configured for fallback search.");
  }
  ...
  if (!response.ok) {
    ...
    throw new Error(
      `YouTube fallback search failed (${response.status}): ${body.slice(0, 160)}`,
    );
  }
...
async function searchYouTubeForSubject(subject: Subject): Promise<SearchResult> {
  try {
    const ids = await searchYouTubeWeb(subject);
    return { ids, source: "web" };
  } catch (webError) {
    const message =
      webError instanceof Error ? webError.message : "Web search failed";
    console.warn(
      "AI IMPORT WEB SEARCH FALLBACK:",
      subject.nameKo || subject.nameEn,
      message,
    );
    const ids = await searchYouTubeApiFallback(subject);
    return { ids, source: "api-fallback" };
  }
}
...
async function loadYouTubeDetails(videoIds: string[]) {
  if (!YOUTUBE_API_KEY) {
    throw new Error("YOUTUBE_API_KEY is not configured for video metadata.");
  }
    ...
    for (const item of payload.items ?? []) {
      ...
      videos.set(id, {
        ...
        durationSeconds: parseDuration(
          String(item.contentDetails?.duration ?? "PT0S"),
        ),
...
async function loadExistingSourceIds(
  ...
  for (let index = 0; index < sourceIds.length; index += 200) {
    ...
    if (worksError) {
      throw new Error(`Existing work lookup failed: ${worksError.message}`);
    }
    if (queueError) {
      throw new Error(`Existing queue lookup failed: ${queueError.message}`);
    }
    for (const row of [
      ...((works ?? []) as any[]),
      ...((queued ?? []) as any[]),
...
function durationScore(duration: number, profile: CategoryStyleProfile) {
  ...
  return Math.round(
    clamp(100 - Math.abs(Math.log((duration + 5) / (target + 5))) * 58),
  );
}
...
function applyHeuristics(
  ...
  const velocities = drafts
    .map(
      (draft) =>
        draft.viewCount / Math.max(0.5, daysAgo(draft.publishedAt)),
    )
    .sort((a, b) => a - b);
  const ratios = drafts
    .map((draft) =>
...
function queueSelection(candidates: RankedCandidate[], maxQueue: number) {
  const eligible = candidates
    .filter((candidate) => {
      ...
      if (
        candidate.ai.contentType === "mv" ||
        candidate.ai.contentType === "news"
      ) {
        return false;
      }
...
export async function collectAiImportCandidatesWeb(
  supabase: SupabaseClient,
  options: CollectorOptions,
) {
  if (!YOUTUBE_API_KEY) {
    throw new Error(
      "YOUTUBE_API_KEY is required for low-cost videos.list metadata lookup.",
    );
  }
  ...
  const maxQueue = Math.max(
    1,
    Math.min(options.maxQueue ?? MAX_QUEUE, MAX_QUEUE),
  );
  ...
  const stats: Record<string, unknown> = {
    ...
    maxQueue,
    searchMode: "youtube-web-first",
  };

  try {
    const [subjects, searchedToday] = await Promise.all([
      loadSubjects(supabase),
      loadTodaySearchedSubjectIds(supabase, dateKey),
    ]);
    const selectedSubjects = selectSubjects(subjects, searchedToday, dateKey);
    ...
    stats.todayAlreadySearchedSubjectCount = searchedToday.size;
    stats.remainingUnsearchedSubjectCount = subjects.filter(
      (subject) => !searchedToday.has(subject.id),
    ).length;
    stats.selectedSubjects = selectedSubjects.map((subject) => ({
      id: subject.id,
      category: subject.category,
      name: subject.nameKo || subject.nameEn || subject.nameZhTw,
    ...
    if (selectedSubjects.length === 0) {
      const result = {
        ...stats,
        searchedSubjectCount: 0,
        queuedCount: 0,
        subjectPoolExhausted: true,
      };
    ...
    const searchResults: Array<{
      subject: Subject;
      ids: string[];
      source: "web" | "api-fallback" | "failed";
      error: string | null;
    }> = [];

    for (let index = 0; index < selectedSubjects.length; index += 3) {
      const group = selectedSubjects.slice(index, index + 3);
      const resultGroup = await Promise.all(
        group.map(async (subject) => {
          try {
            ...
            console.error(
              "AI IMPORT SUBJECT SEARCH ERROR:",
              subject.nameKo || subject.nameEn,
              message,
            );
            return {
      ...
      if (index + 3 < selectedSubjects.length) {
    ...
    let webSearchSubjectCount = 0;
    let apiFallbackSubjectCount = 0;
    for (const result of searchResults) {
      if (result.error) searchErrorCount += 1;
      if (result.source === "web") webSearchSubjectCount += 1;
      if (result.source === "api-fallback") apiFallbackSubjectCount += 1;
    ...
    stats.webSearchSubjectCount = webSearchSubjectCount;
    stats.apiFallbackSubjectCount = apiFallbackSubjectCount;
    ...
    stats.existingOrPreviouslyQueuedVideoCount = existing.size;

    const rawDrafts: Array<
      YoutubeVideo & { subject: Subject; subjectMatchScore: number }
    > = [];
    ...
    for (const [videoId, subjectsForVideo] of hitsByVideoId.entries()) {
      ...
      if (
        ...
        daysAgo(video.publishedAt) > LOOKBACK_DAYS + 1
      ) {
        continue;
      }
    ...
    const aiInputs: CandidateForAi[] = candidates.map((candidate) => ({
      ...
      subjectName:
        candidate.subject.nameKo ||
        candidate.subject.nameEn ||
        candidate.subject.nameZhTw,
    ...
    for (const judgment of aiResult.judgments) {
    ...
    const blockedAtInsert = await loadExistingSourceIds(
      supabase,
      selected.map((candidate) => candidate.id),
    );
    const finalSelected = selected.filter(
      (candidate) => !blockedAtInsert.has(candidate.id),
    );
    stats.duplicateRaceFilteredCount = selected.length - finalSelected.length;

    const rows = finalSelected.map((candidate) => ({
      category: candidate.category,
      source: "youtube",
    ...
    if (rows.length > 0) {
      ...
      for (const row of (inserted ?? []) as any[]) {
        if (typeof row.source_id === "string") {
          insertedSourceIds.add(row.source_id);
        }
      }
    }

    const actuallyQueued = finalSelected.filter((candidate) =>
      insertedSourceIds.has(candidate.id),
    );
    let queuedKpop = 0;
    let queuedCheer = 0;
    for (const candidate of actuallyQueued) {
    ...
    stats.selectedForQueueCount = finalSelected.length;
    stats.queuedCount = actuallyQueued.length;
    stats.queuedByCategory = { kpop: queuedKpop, cheer: queuedCheer };
    stats.topQueued = actuallyQueued.slice(0, 15).map((candidate) => ({
