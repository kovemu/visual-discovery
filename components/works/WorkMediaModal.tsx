...
const SHARE_CLIP_BASE_URL = "https://kovemu.com";
  ...
  async function handleCopyLink() {
    try {
      await navigator.clipboard.writeText(
        `${SHARE_CLIP_BASE_URL}/clip/${work.id}`,
      );
      setCopied(true);
    } catch (error) {
      console.error("COPY CLIP LINK ERROR:", error);
    }
  }
    ...
    >
      ...
      >
        ...
        <aside className="relative w-full shrink-0 bg-[#111111] px-4 py-3 md:w-[300px] md:border-l md:border-white/10 md:p-6">
          ...
          <div className="flex flex-col gap-3 md:absolute md:left-6 md:right-6 md:top-[43%] md:w-auto md:-translate-y-1/2 md:gap-4">
            <button
              type="button"
              onClick={() => {
                void handleCopyLink();
              }}
              className="flex h-9 w-full items-center justify-center gap-1.5 rounded-full border border-white/15 bg-transparent text-xs font-medium text-white/70 transition hover:border-white/25 hover:bg-white/5 hover:text-white/90"
            >
              {copied ? (
                <>
            ...
            <button
              ...
              className={`flex h-11 w-full items-center justify-center gap-2 rounded-full border text-sm font-semibold transition ${
                isSaved
